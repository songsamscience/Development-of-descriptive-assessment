...
function repArr(name) { if (!state.reps[name]) state.reps[name] = []; return state.reps[name]; }
...
function renderRep(name) {
  ...
  if (name === 'planLesson') {
    ...
    box.innerHTML = arr.map(function (r, i) {
      return '<div class="rep-item"><div class="rep-idx">' + (i + 1) + '</div><div class="rep-body">' +
        cfg.fields.map(function (f) {
          return '<input type="text" data-i="' + i + '" data-f="' + f.k + '" value="' + esc(r[f.k] || '') + '" placeholder="' + esc(f.ph) + '">';
        }).join('') +
...
function initReps() {
  ...
  $$('[data-add]').forEach(function (b) {
    b.addEventListener('click', function () {
      ...
      var arr = repArr(name);
      arr.push(name === 'planLesson' ? { no: arr.length + 1, act: '', eval: '', mean: '' } : { text: '' });
...
function initChecks() {
  ...
  [['#procExpand', 'proc'], ['#itemExpand', 'item'], ['#rubExpand', 'rubric']].forEach(function (p) {
...
function buildPrompt() {
  ...
  var first = F('a_first') || '어느 학년, 어느 단원으로 만드시겠습니까?';
  ...
  t.push('- 완성된 문항 전체를 교사의 결정 없이 먼저 내놓지 않는다.');
  ...
  t.push('# 첫 문장');
  t.push('대화를 시작할 때 다른 인사말 없이 다음 문장 하나만 말한다.');
  t.push('“' + first + '”');
...
function prepareForWord(src) {
  ...
  Array.prototype.slice.call(c.querySelectorAll('.plat-tabs, .progress-bar, .ring, #tabInk')).forEach(function (n) {
    n.parentNode && n.parentNode.removeChild(n);
  });
...
function initActions() {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (meta && e.key >= '1' && e.key <= '5') { e.preventDefault(); goTab('p' + e.key); }
  });
}
function legacyCopy(txt) {
...
function init() {
  ...
  renderRubric();
  renderPlanDoc(); renderItemDoc(); renderRubricDoc();
  buildPrompt();
